...
setwd("C:/Users/angelo/GitAngelo/R/Titulos Publicos") #talvez  tenha que mudar
...
tammenor<-5
Basevenda<-matrix(0,ncol=tammenor)
Basecompra<-matrix(0,ncol=tammenor+1)
i<-1
while(i<=tam){
  j=1
  baselinha<-0
  tamnode<-length(xml_find_all(Tesouro.nodes[i], ".//td"))
  while(j<=tamnode){
    dadoini<-substr(xml_find_all(Tesouro.nodes[i], ".//td")[j], str_locate(xml_find_all(Tesouro.nodes[i], ".//td")[j], "listing")[,2]+3,str_locate(xml_find_all(Tesouro.nodes[i], ".//td")[j], "</td>")[,1]-1)
    #limpar os caracteres errados
    dadoini<-as.character(gsub(",",".",gsub("[R$]","",gsub("[.]","",gsub(">","",dadoini)))))
    baselinha<-cbind(baselinha, dadoini)
    j=j+1
  }
  if(length(baselinha)==tammenor){
    Basevenda<-rbind(Basevenda,baselinha)
  }else{
    Basecompra<-rbind(Basecompra,baselinha)
  }
  i=i+1
}
#eliminar as linhas e colunas zeradas
Basevenda<-as.data.frame(Basevenda[-1,-1])
Basecompra<-as.data.frame(Basecompra[-1,-1])
#Renomear
names(Basevenda) <-  c("Titulo", "Vencimento", "Taxa de Venda","Valor de Venda")
names(Basecompra) <-  c("Titulo", "Vencimento", "Taxa de Compra","Valor Unitario","Valor de Compra")
#Merge
